...
class Simulation(DirectObject):
  ...
  def processInput(self, dt):
    ...
    if inputState.isSet('forward'): force.setY(0.1)
    if inputState.isSet('reverse'): force.setY(-0.1)
    if inputState.isSet('left'):    force.setX(-0.1)
    if inputState.isSet('right'):   force.setX( 0.1)
    if inputState.isSet('turnLeft'):  torque.setZ( 0.1)
    if inputState.isSet('turnRight'): torque.setZ(-0.1)
  ...
  def setup(self):
    ...
    #Rocket
    shape = BulletCylinderShape(0.2, 2, ZUp)

    self.rocketNP = self.worldNP.attachNewNode(BulletRigidBodyNode('Cylinder'))
    #self.rocketNP.node().setMass(1.0)
    self.rocketNP.node().addShape(shape)
    self.rocketNP.setPos(0, 0, 2)
    self.rocketNP.setCollideMask(BitMask32.allOn())

    self.world.attachRigidBody(self.rocketNP.node())
    
    for i in range(4):
      leg = BulletCylinderShape(0.02, 1, XUp)
      self.rocketNP.node().addShape(leg, TransformState.makePosHpr(Vec3(0.6*math.cos(i*math.pi/2),0.6*math.sin(i*math.pi/2),-1.2),Vec3(i*90,0,30)))
    
    
    shape =  BulletConeShape(0.15,0.3, ZUp)
    
    self.rocketNozzle  = self.worldNP.attachNewNode(BulletRigidBodyNode('Cone'))
    self.rocketNozzle.node().setMass(0.2)
    self.rocketNozzle.node().addShape(shape)
    self.rocketNozzle.setPos(0,0,0.8)
    self.rocketNozzle.setCollideMask(BitMask32.allOn())

    self.world.attachRigidBody(self.rocketNozzle.node())
    
    frameA = TransformState.makePosHpr(Point3(0, 0, -1), Vec3(0, 0, 90))
    frameB = TransformState.makePosHpr(Point3(0, 0, 0.2), Vec3(180, 0, 90))

    cone = BulletConeTwistConstraint(self.rocketNP.node(), self.rocketNozzle.node(), frameA, frameB)
    #cone.enableMotor(1)
    cone.setDebugDrawSize(2.0)
    cone.setLimit(20, 20, 0, softness=1.0, bias=0.3, relaxation=1.0)
