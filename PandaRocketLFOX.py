...
class Simulation(ShowBase):
    ...
    def setup(self):
        ...
        self.LH2NP = self.worldNP.attachNewNode(BulletRigidBodyNode('Cone'))
        self.LH2NP.node().setMass(self.fuelAmount_LH2)
        self.LH2NP.node().addShape(shape)
        self.LH2NP.setPos(0, 0, self.rocketNP.getPos().getZ()-self.LH2Center)
        self.LH2NP.setCollideMask(BitMask32.allOn())
        self.LH2NP.node().setCollisionResponse(0)

        self.LOXNP = self.worldNP.attachNewNode(BulletRigidBodyNode('Cone'))
        self.LOXNP.node().setMass(self.fuelAmount_LOX)
        self.LOXNP.node().addShape(shape)
        self.LOXNP.setPos(0, 0, self.rocketNP.getPos().getZ()-self.LOXCenter)
        ...
        frameC = TransformState.makePosHpr(Point3(0, 0, 0), Vec3(0, 0, 90))

        self.sliderLH2 = BulletSliderConstraint(self.rocketNP.node(), self.LH2NP.node(), frameA, frameB, 1)
        self.sliderLH2.setTargetLinearMotorVelocity(0)
        self.sliderLH2.setDebugDrawSize(2.0)
        self.sliderLH2.set_lower_linear_limit(0)
        self.sliderLH2.set_upper_linear_limit(0)
        self.world.attachConstraint(self.sliderLH2)

        self.sliderLOX = BulletSliderConstraint(self.rocketNP.node(), self.LOXNP.node(), frameA, frameC, 1)
        self.sliderLOX.setTargetLinearMotorVelocity(0)
        self.sliderLOX.setDebugDrawSize(2.0)
        self.sliderLOX.set_lower_linear_limit(0)
        self.sliderLOX.set_upper_linear_limit(0)
        self.world.attachConstraint(self.sliderLOX)
        ...
        self.sliderLH2.set_lower_linear_limit(-self.LH2Center - 0.5 * self.LH2TankLength * (1 - self.fuel_LH2))
        self.sliderLOX.set_lower_linear_limit(-self.LOXCenter - 0.5 * self.LOXTankLength * (1 - self.fuel_LOX))

        self.sliderLH2.set_upper_linear_limit(-self.LH2Center + 0.5 * self.LH2TankLength * (1 - self.fuel_LH2))
        self.sliderLOX.set_upper_linear_limit(-self.LOXCenter + 0.5 * self.LOXTankLength * (1 - self.fuel_LOX))

        for i in range(20):
            self.world.doPhysics(self.dt, 5, 1.0 / 180.0)
            self.rocketNP.node().applyForce(Vec3(0,0,1000000), Vec3(0, 0, 0))
            self.LH2NP.node().applyForce(Vec3(0,0,-200000), Vec3(0,0,0))
            self.LOXNP.node().applyForce(Vec3(0, 0, -100000), Vec3(0, 0, 0))

        for i in range(40):
            self.world.doPhysics(self.dt, 5, 1.0 / 180.0)
            self.rocketNP.node().applyForce(Vec3(0, 0, 100000), Vec3(0, 0, 0))
    ...
    def updateRocket(self, LH2dot, LOXdot, dt):
        ...
        I2 = 1 / 4 * self.fuelAmount_LH2 * self.fuelRadius ** 2 + 1 / 12 * self.fuelAmount_LH2 * LH2height * LH2height
        self.LH2NP.node().setInertia(Vec3(I2, I2, I1))

        I1 = 1 / 2 * self.fuelAmount_LOX * self.fuelRadius ** 2
        I2 = 1 / 4 * self.fuelAmount_LOX * self.fuelRadius ** 2 + 1 / 12 * self.fuelAmount_LOX * LOXheight * LOXheight
        self.LOXNP.node().setInertia(Vec3(I2, I2, I1))

        #print(self.LH2Center,self.LOXCenter)
        #print(self.LH2TankLength,self.LOXTankLength)
