...
#                                  optepochs=4,ent_coef=0.01,verbose=1, tensorboard_log="./rocket_tensorboard/",
...
for i in range(1000):
    ...
    alt_reward.append(abs(Or_obs[0][0])/200)
    mix_reward.append(abs(Or_obs[0][6]/Or_obs[0][5]-5.5)/50)
    temp_reward.append(abs(Or_obs[0][4]-900)/10000)
...
plt.plot(time, data[8], ':', label='LOX Command')
plt.plot(time, data[9], ':', label='LH2 Command')
plt.plot(time, data[10], ':', label='Mix Command')

plt.plot(time, actions[0], label='LOX Valve')
plt.plot(time, actions[1], label='LH2 Valve')
plt.plot(time, actions[2], label='Mix Valve')
plt.legend(loc='best')
